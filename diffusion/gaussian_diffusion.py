...
from model.motion_diffusion_ae import MoDiffAE, ControlConfig
...
class GaussianDiffusion:
    ...
    def latent_kl_divergence_loss(self, mu: torch.Tensor, logvar: torch.Tensor):
        """ Standard VAE KL Divergence for N(0, 1) prior """
        kl = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp(), dim=-1) # (time, bs)
        return kl.mean(dim=0) # (bs,)
    ...
    def p_mean_variance(
        ...
        if get_layer_activation > -1 and get_layer_activation < model.model.num_layers:
            layer_act_hook = [get_layer_activation]
        
        out_dict, db_dict = model(x, self._scale_timesteps(t), get_layer_activation=layer_act_hook, **model_kwargs)
        model_output = out_dict['out']
        if isinstance(model.model, MoDiffAE) and 'z_sem' not in model_kwargs['y'].keys():
            model_kwargs['y']['z_sem'] = out_dict['z_sem'] # cache option
    ...
    def p_sample_dift(
        ...
        # print('nonzero_mask', nonzero_mask.shape, nonzero_mask)
        sample = out["mean"] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise
        return {"sample": sample, "pred_xstart": out["pred_xstart"], "db_dict": out["db_dict"]}
    ...
    def p_sample_single_timestep(
        ...
        with th.no_grad():
            ...
            if get_activations["timestep"] == t:
                activations_dict[t] = out["db_dict"]["activations"]
    ...
    def ddim_inversion_loop(
        self,
        model,
        img, # clean input (x_0) to be inverted
        clip_denoised=True,
        denoised_fn=None,
        model_kwargs=None,
        device=None,
        ...
        # In inversion, noise is the 'clean' starting point
        assert img is not None, "Inversion requires input data passed"
        batch_size = img.shape[0]
        indices = list(range(skip_timesteps, self.num_timesteps))
        ...
        for i in indices:
            ...
            with th.no_grad():
                out = self.ddim_reverse_sample(
                    ...
                    denoised_fn=denoised_fn,
                    model_kwargs=model_kwargs,
                    eta=0.0, # Internal requirement for deterministic inversion
                )
                img = out["sample"]
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if isinstance(model.model, (MoDiffAE)):
                # add MMix training args forward
                model_kwargs['control'] = ControlConfig(
                    x = x_start.clone().unsqueeze(1), # add x_start clean motion as control signal for MMix
                    y = model_kwargs['y'],            # the y stays the same
                )
            
            # AnyTop forward
            out_dict, db_dict = model(x_t, self._scale_timesteps(t), **model_kwargs)
            model_output = out_dict['out']
            ...
            if self.lambda_kl > 0.:
                terms["kl_loss"] = self.latent_kl_divergence_loss(out_dict['mu'], out_dict['logvar'])
                terms["loss"] = terms["loss"] + self.lambda_kl * terms["kl_loss"]

...
class TrainLoop:
    ...
    def run_loop(self):
         ...
         while self.total_step() < self.num_steps:
            epochs_so_far += 1
            print(f'Starting epoch [{epochs_so_far}] at step {self.total_step()}')
            for motion, cond in tqdm(self.data):
                ...
                if self.total_step() % self.save_interval == 0 or self.total_step() == self.num_steps - 1:
                    ...
                    self.model.eval()
                    self.evaluate()
                    if self.args.model_name in ['MoDiffAE']:
    ...
    def _anneal_lambdas(self):
        curr_step = self.total_step()
        target = self.args.lambda_kl
        if not self.args.kl_bottleneck :
            self.diffusion.lambda_kl = 0.0
            return
        if curr_step < self.args.kl_warmup_steps:
            # Warmup Phase (Stay at 0)
            new_lambda = 0.0
        elif curr_step < (self.args.kl_warmup_steps + self.args.kl_anneal_steps):
            # Annealing Phase (Ramp from 0 to Target)
            progress = (curr_step - self.args.kl_warmup_steps) / self.args.kl_anneal_steps
            new_lambda = progress * target  
        else:
            # Steady State (Stay at Target)
            new_lambda = target
        self.diffusion.lambda_kl = new_lambda

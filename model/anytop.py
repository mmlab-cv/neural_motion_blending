...
from model.modules.motion_transformer_anytop import GraphMotionDecoderLayer, GraphMotionDecoder
...
class AnyTop(nn.Module):
    ...
    def forward(self, x, timesteps, get_layer_activation=[], y=None):
        """
        ...
        output, activations = self.seqTransDecoder(tgt=x, timesteps_embs=timesteps_emb, memory=None, spatial_mask=spatial_mask, temporal_mask = temporal_mask, y=y, get_layer_activation=get_layer_activation)
        output = self.output_process(output) # Applies linear layer on each frame to convert it back to feature len dim
        
        return {'out': output}, {'activations': activations}
